...
def create():
    ...
    log_file_path = definitions.DIR_LOGS + "/" + log_file_name
...
def analysis(exp_id):
    size_space, n_enumerated, n_plausible, n_noncompile = values.ANALYSIS_RESULTS[
        exp_id
    ]
...
def end(time_duration, is_error=False):
    ...
    output("Patch Start Count: " + str(values.COUNT_INITIAL))
    # output("Patch End Seed Count: " + str(values.COUNT_PATCH_END_SEED))
    output("Patch End Count: " + str(values.COUNT_VALID))
