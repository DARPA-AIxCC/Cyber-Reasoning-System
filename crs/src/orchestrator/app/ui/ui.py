...
class Cerberus(App[List[Result]]):
    ...
    def on_mount(self) -> None:
        ...
        asyncio.get_running_loop().run_in_executor(None, self.prepare_tasks_run, loop)
    ...
    def prepare_subjects(
        ...
        def prepare_subjects_job(
            ...
            try:
                ...
                complete_images.put(
                    (experiment_item[definitions.KEY_ID], job_identifier, True)
                )
            except Exception as e:
                emitter.information(
                ...
                complete_images.put(
                    (experiment_item[definitions.KEY_ID], job_identifier, False)
                )
            finally:
    ...
    def prepare_tool_subjects(
        ...
        def prepare_tool_subjects_job(
            ...
            try:
                ...
                complete_images.put(
                    (experiment_item[definitions.KEY_ID], job_identifier, None, False)
                )
            finally:
                self.cpu_queue.put(cpu, block=False)
            emitter.information(
    ...
    async def on_job_allocate(self, message: JobAllocate) -> None:
        ...
        def job_allocated_job() -> None:
            ...
            job_time_map[message.identifier] = (start_time, timeout, message.tool)
            job_time_map_mutex.release()
            ...
            ).add_row(*row_data, key=message.identifier)
    ...
    async def on_job_finish(self, message: JobFinish) -> None:
        ...
        try:
            ...
            row_key = finished_subjects_table.add_row(
                *message.row_data, key=message.key
            )
