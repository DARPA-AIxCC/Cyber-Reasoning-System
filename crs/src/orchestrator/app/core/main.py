...
def main() -> None:
    os.system("bash -c 'echo PROCESSES ARE `nproc`, version is 1.2.8'")
    pid = os.getpid()
    os.system(f"choom --adjust -100 -p {pid}")
...
def process_config_file(parsed_args: Namespace) -> Config:
    ...
    config_loader = ConfigDataLoader(
        file_path=parsed_args.config_file, validation_schema=config_validation_schema
    )
    config_loader.load()
...
def process_tasks(tasks: TaskList) -> bool:
    ...
    for iteration, (task_config, task_data) in enumerate(tasks):
        ...
        image_name = create_task_image_identifier(
            benchmark, tool, experiment_item, tool_tag
        )
        if task_config.task_type != "composite":
