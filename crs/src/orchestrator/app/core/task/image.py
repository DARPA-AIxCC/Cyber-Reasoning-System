...
def _tool_based_image(
    ...
    utilities.error_exit("BOOM. UNSUPPORTED")
    ...
    tmp_dockerfile = join(dir_info["local"]["setup"], dockerfile_name)
...
def _subject_based_image(
    ...
    with open(tmp_dockerfile, "w") as dock_file:
        ...
        with open(join(list(tool.bindings.keys())[0], "Dockerfile"), "r") as steps:
            for line in steps:
