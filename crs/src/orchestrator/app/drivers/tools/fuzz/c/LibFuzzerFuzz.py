...
class LibFuzzerFuzz(AbstractFuzzTool):
    def __init__(self) -> None:
        self.name = os.path.basename(__file__)[:-3].lower()
        self.bindings = {
            join(values.dir_main, "libfuzzer_fuzz"): {"bind": "/tool", "mode": "rw"}
        }
    ...
    def invoke(
        ...
        tool_folder = (
            join(values.dir_main, "libfuzzer_fuzz") if self.locally_running else "/tool"
        )
    ...
    def save_artifacts(self, dir_info: Dict[str, str]) -> None:
        ...
        """
        super(LibFuzzerFuzz, self).save_artifacts(dir_info)
