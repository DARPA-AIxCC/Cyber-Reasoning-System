...
class KernelFuzz(AbstractFuzzTool):
    ...
    def invoke(
        ...
        bug_info["cp_src"] = join(
            self.dir_expr, "src", "src", bug_info["cp_sources"][0]["name"]
        )
        ...
        status = self.run_command(
            f"bash -c 'python3 {join(tool_folder,'crs_entrypoint.py')} {join(self.dir_base_expr,'meta-data.json')}'",
    ...
    def save_artifacts(self, dir_info: Dict[str, str]) -> None:
        ...
        """
        super(KernelFuzz, self).save_artifacts(dir_info)
