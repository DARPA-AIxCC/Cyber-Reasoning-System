...
class Hermes(AbstractRepairTool):
    def __init__(self) -> None:
        self.name = os.path.basename(__file__)[:-3].lower()
        super(Hermes, self).__init__(self.name)
        self.image_user = "ubuntu"
        self.runs_as_root = False
        self.image_name = "rshariffdeen/hermes"
    ...
    def create_meta_data(self) -> None:
        ...
        if self.is_file(patch_info_json):
            ...
            if isinstance(patch_info, Dict):
                for _p in patch_info["patches"]:
                    _p["generator"] = self.name
    ...
    def generate_config_file(self, bug_info: Dict[str, Any]) -> str:
        ...
        for result in stack_trace[:stack_limit]:
            source_file, line_number = result.split(":")
            if src_dir in source_file:
                source_file = source_file.replace(src_dir + "/", "")
            fix_locations.append(
                {"source_path": source_file, "line_number": int(line_number)}
            )
        ...
        for result in localization_list:
            ...
            for _l in line_numbers:
                fix_locations.append(
                    {"source_path": source_file, "line_number": int(_l)}
                )
    ...
    def create_api_auth(self) -> None:
        config_file_path = f"{self.tool_folder}/config.toml"
        google_auth_json_path = f"{self.tool_folder}/google-key.json"
        google_auth_data = {
    ...
    def invoke(
        ...
        self.tool_folder = (
            join(values.dir_main, "hermes")
            if self.locally_running
            else "/home/ubuntu/hermes"
        )
        
        if self.key_localization not in bug_info:
            self.emit_error("No localization. Aborting Hermes")
        ...
        repair_command = (
            f"timeout -k 5m {timeout}h python3 multi_location.py {self.config_file}"
        )
    ...
    def save_artifacts(self, dir_info: Dict[str, str]) -> None:
        ...
        if self.locally_running:
            run_as_sudo = False
        self.run_command(copy_cmd, run_as_sudo=run_as_sudo)
        super(Hermes, self).save_artifacts(dir_info)
        return
