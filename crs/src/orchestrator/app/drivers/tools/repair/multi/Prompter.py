...
class Prompter(AbstractRepairTool):
    ...
    def invoke(
        ...
        self.tool_folder = join(values.dir_main, "prompter") if self.locally_running else "/home/ubuntu/prompter"
        ...
        self.create_config()
        file_path = bug_info[self.key_localization][0][self.key_fix_file]
        if self.dir_expr not in file_path:
            file_path = f"{self.dir_expr}/src/{file_path}"
        ...
        repair_command = f"timeout -k 5m {timeout}h python3 cli.py {file_path} {language} {cwe_id} {fix_line} {self.output_path}"
        self.emit_debug(repair_command)
        status = self.run_command(
            repair_command, self.log_output_path, self.tool_folder
        )
