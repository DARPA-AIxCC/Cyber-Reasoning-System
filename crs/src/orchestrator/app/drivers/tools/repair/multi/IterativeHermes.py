...
from app.drivers.tools.repair.multi.Hermes import Hermes


class IterativeHermes(Hermes):
    def __init__(self) -> None:
        super(IterativeHermes, self).__init__()
    ...
    def generate_config_file(self, bug_info: Dict[str, Any]) -> str:
        ...
        config_object["patch_dir"] = join(self.dir_setup, "patches")
        fix_locations = []
        ...
        for _p in patch_list:
            ...
            if _p_id in validation_result:
                _result_id = validation_result[_p_id]
    ...
    def invoke(
        ...
        repair_command = (
            f"timeout -k 5m {timeout}h python3 feedback.py {self.config_file}"
        )
