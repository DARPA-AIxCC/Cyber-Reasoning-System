...
class BasicWorkflow(AbstractCompositeTool):
    ...
    def invoke_advanced(
        ...
        root_dir = join(
            values.dir_composite_workspace, "run-{}".format(hash.hexdigest()[:10])
        )
    ...
    def run_subtask(
        ...
        new_mappings = self.make_task_mappings(
            tool.name, hash.hexdigest()[:10], self.root_task_mappings, tool.tool_tag
    ...
    def on_fuzzing_finished(self, base_dir: str) -> None:
        try:
            ...
            subtask_tag = subtask_hash.hexdigest()[:10]
    ...
    def on_validation_finished(self, event: FileSystemEvent) -> None:
        def check(patch_path: str, vulnerability_id: str) -> Tuple[bool, str]:

            base64_input = ""
            with open(patch_path, "rb") as f:
                base64_input = base64.encodebytes(f.read())
            patch_input = {
                "cpv_uuid": vulnerability_id,
                "data": base64_input.decode("ascii").strip(),
            }
            with open(patch_path, "r") as f:
                patch = f.read()
                self.emit_debug(f"[STEP] Patch for {vulnerability_id} is:\n{patch}")
            self.emit_debug(
                f"[STEP] Sending input {patch_input} for {vulnerability_id}"
            )

            if os.getenv("AIXCC_API_HOSTNAME") or os.getenv("HEALING_TOUCH_IAPI"):
                # with self.patch_validation_map[vulnerability_id]:
                gp_uuid_received = False
                gp_uuid = None
                while not gp_uuid_received:
                    (success, patch_response) = self.try_call(
                        "submission/gp/", patch_input
                    )
                    if not success:
                        continue

                    if patch_response["status"] == "rejected":
                        self.emit_highlight(f"Patch rejected")
                        return (False, None)

                    gp_uuid = str(patch_response["gp_uuid"])
                    gp_uuid_received = True

                gp_validated = False
                while not gp_validated:
                    (success, patch_decision) = self.try_call(
                        f"submission/gp/{gp_uuid}", {}, "get"
                    )
                    if not success:
                        time.sleep(0.5)
                        continue

                    if patch_decision["status"] == "pending":
                        time.sleep(5)
                        continue
                    if patch_decision["status"] == "rejected":
                        return (False, None)

                    if patch_decision["status"] == "accepted":
                        return (True, gp_uuid)

                    self.emit_highlight(f"Got data {patch_decision}")
                    return (False, None)
            else:
                import uuid
        ...
        vulnerability_id = bug_info.get("cpv_uuid", "")
        ...
        patch_path = plausible_patches[-1]
        ...
        with self.patch_validation_map[vulnerability_id][0]:
            successful, identifier = check(patch_path, vulnerability_id)
            if successful:
                with self.patch_validation_map_lock:
                    self.patch_validation_map[vulnerability_id] = (
                        self.patch_validation_map[vulnerability_id][0],
                        True,
                    )
                self.emit_warning("Vulnerability submission was successful. Success!")
            else:
                self.emit_warning(
    ...
    def on_bisection_finished(self, event: FileSystemEvent) -> None:
        def check(
            test_path: str, sanitizer: str, name: str, harness: str, commit: str
        ) -> Tuple[bool, str]:
            ...
            vulnerability_input = {
                "cp_name": name,
                "pou": {"commit_sha1": commit, "sanitizer": sanitizer},
                "pov": {
                    "harness": harness["id"],
                    "data": base64_input.decode("ascii").strip(),
                },
            }
            self.emit_debug("Sending input {}".format(vulnerability_input))

            if os.getenv("AIXCC_API_HOSTNAME") or os.getenv("HEALING_TOUCH_IAPI"):
                received_vd_uuid = False
                vd_uuid = None
                while not received_vd_uuid:
                    (received, data) = self.try_call(
                        "submission/vds/", vulnerability_input
                    )
                    if not received:
                        continue

                    if data["status"] == "rejected":
                        self.emit_highlight(f"Vulnerability rejected, possibly cached")
                        return (False, None)

                    vd_uuid = str(data["vd_uuid"])
                    received_vd_uuid = True

                received_cpv_uuid = False
                while not received_cpv_uuid:
                    # TODO Check out how request volume
                    (received, vulnerability_decision) = self.try_call(
                        f"submission/vds/{vd_uuid}", {}, "get"
                    )
                    if not received:
                        continue
                    if vulnerability_decision["status"] == "pending":
                        time.sleep(5)
                        continue
                    if vulnerability_decision["status"] == "rejected":
                        self.emit_highlight(f"Vulnerability rejected :(")
                        return (False, None)

                    if vulnerability_decision["status"] == "accepted":
                        self.emit_highlight(
                            f"Vulnerability confirmed! {vulnerability_decision['cpv_uuid']}"
                        )
                        return (True, vulnerability_decision["cpv_uuid"])

                    self.emit_highlight(f"Got response {vulnerability_decision}")
                    return (False, None)
        ...
        with self.vulnerability_validation_map[failure_signature][0]:
            found_new_vulnerability, identifier = check(
                failing_test, sanitizer, bug_info["name"], harness, commit
            )
            if found_new_vulnerability:
                ...
                def inject_id(base_setup, enhanced_setup, bug_info):
                    bug_info["cpv_uuid"] = identifier
                    pass

                self.on_task_finished(event, ["localize", "repair"], inject_id)
            else:
                self.emit_warning(
                    "Vulnerability submission was not successful. Please check why"
                )
    ...
    def try_call(
        self, path: str, data: Dict[str, Any], verb: Literal["get", "post"] = "post"
    ):
        username = os.getenv("CAPI_ID", "00000000-0000-0000-0000-000000000000")
        password = os.getenv("CAPI_TOKEN", "secret")
        loc = f"{os.getenv('AIXCC_API_HOSTNAME')}/{path}"
        alt_loc = (
            # f"{os.getenv('HEALING_TOUCH_IAPI')}/{path}"
            # if os.getenv("HEALING_TOUCH_IAPI")
            # else 
            None
        )

        def do_call(site):
            self.emit_debug(f"sending request to {site}")
            if verb == "post":
                ...
                submission = httpx.get(site, auth=(username, password))
            return submission

        submission = do_call(alt_loc if alt_loc else loc)
        try:
            res = submission.json()
            return True, res
        except Exception as e:
            if alt_loc:
                self.emit_warning(
                    "Failed with alternative location. Trying with main one"
                )
                submission = do_call(loc)
                try:
                    res = submission.json()
                    return True, res
                except Exception as e:
    ...
    def get_setup_directories(self, root: str, subtask_tag: str) -> Tuple[str, str]:
        ...
        if os.path.isfile(join(root, definitions.INTERNAL_METADATA_JSON)):
            ...
            with open(join(root, definitions.INTERNAL_METADATA_JSON), "r") as f:
                ...
                if len(previous_setup.split("-")[-1]) == 10:
                    # The last argument is either the run index or the tool tag and for the run index to be reaching 10 digits, that is suspicious
                    enhanced_setup = join(
                        self.root_setups_dir, f"{previous_setup[:-11]}-{subtask_tag}"
                    )
                else:
                    enhanced_setup = join(
