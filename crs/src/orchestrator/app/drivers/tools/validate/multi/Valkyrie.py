...
class Valkyrie(AbstractValidateTool):
    ...
    def populate_config_file(self, bug_info: Dict[str, Any]) -> str:
        ...
        conf_content = list()
        dir_src = f"{self.dir_expr}/src/src/{bug_info['cp_sources'][-1]['name']}"
        ...
        test_id_list = [
            f"tests/{x}" for x in bug_info[self.key_failing_test_identifiers]
        ]
        t = lambda x: f'"{x}"'
        conf_content.append(f"test_id_list:[{','.join(map(t,test_id_list))}]\n")
        ...
        conf_content.append("patch_per_dir_limit:300\n")
    ...
    def invoke(
        ...
        validate_command = (
            "timeout -k 5m {0}h python3 Valkyrie.py --conf=".format(timeout)
            + conf_path
            + " --only-validate "
    ...
    def transform_patches(self, bug_info) -> None:
        input_patches_dir = f"{self.dir_setup}/patches"
