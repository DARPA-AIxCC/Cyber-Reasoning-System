...
def get_enum_max(enum_class):
    if issubclass(enum_class, IntEnum):
        ...
        raise RuntimeError(
            "Invalid type passed to get_enum_max: " + enum_class.__name__
        )
...
def get_sanitized_enum(reader, enum_class):
    ...
    if issubclass(enum_class, IntEnum):
        ...
        raise RuntimeError(
            "Invalid type passed to get_sanitized_enum: " + enum_class.__name__
        )
...
def get_decomposed_flags(combined_flags):
    ...
    flag_str = "|".join(
        [v.name for v in enum_class if v in combined_flags and v.value != 0]
    )
    if flag_str == "":
...
def get_combined_flags(flag_str, enum_class):
    return functools.reduce(
        lambda x, y: x | y, [enum_class[v] for v in flag_str.split("|")]
    )
...
def get_disasm(zydis_info, machine_mode, stack_width, payload):
    ...
    proc = Popen(
        [zydis_info, arg_machine_mode, arg_stack_width, payload[:30]],
        stdout=PIPE,
        stderr=PIPE,
    )
...
def convert_enc_crash_to_json(crash, return_dict=False):
    ...
    prefixes = ZydisInstructionAttributes(
        reader.read_uint64() & ZYDIS_ENCODABLE_PREFIXES
    )
    ...
    for i in range(ZYDIS_ENCODER_MAX_OPERANDS):
        ...
        op_type = ZydisOperandType(
            ZydisOperandType.ZYDIS_OPERAND_TYPE_REGISTER.value
        ...
        if op_type == ZydisOperandType.ZYDIS_OPERAND_TYPE_REGISTER:
            op["reg"] = {"value": reg_value.name, "is4": reg_is4}
            ...
            op["ptr"] = {"segment": ptr_segment, "offset": ptr_offset}
        elif op_type == ZydisOperandType.ZYDIS_OPERAND_TYPE_IMMEDIATE:
            op["imm"] = {"value": str(imm)}
        else:
...
def convert_enc_json_to_crash(test_case_json, from_dict=False):
    ...
    writer.write_uint32(
        get_combined_flags(test_case["allowed_encodings"], ZydisEncodableEncoding)
    )
    writer.write_uint32(ZydisMnemonic[test_case["mnemonic"]])
    writer.write_padding(4)
    writer.write_uint64(
        get_combined_flags(test_case["prefixes"], ZydisInstructionAttributes)
    )
    ...
    for i in range(ZYDIS_ENCODER_MAX_OPERANDS):
        ...
        writer.write_uint32(
            op_type - ZydisOperandType.ZYDIS_OPERAND_TYPE_REGISTER.value
        )
        if op_type == ZydisOperandType.ZYDIS_OPERAND_TYPE_REGISTER:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Provides two-way conversion between crash files and human-readable test cases"
    )
