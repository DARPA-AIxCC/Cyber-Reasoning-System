...
def glob_in_dir(pattern: str, root_dir: Path):
    cwd = os.getcwd()
...
def find_include_path(include: str, search_paths: List[Path]) -> Path:
...
def merge_headers(
    *,
    ...
    include_stack = []
    if stack:
        include_stack = ["//", "// Include stack:", *(f"//   - {x}" for x in stack)]

    filtered = [f"", f"//", f"// Header: {header}", *include_stack, f"//", f""]
...
def merge_sources(*, source_dir: Path, covered_headers: Set[Path]):
    ...
    for source_file in glob_in_dir("**/*.c", source_dir):
        ...
        output += [f"", f"//", f"// Source file: {source_file}", f"//", f""]
    ...
    with open(OUTPUT_DIR / "Zydis.c", "w") as f:
        f.write(
            "\n".join(
                FILE_HEADER
                + merge_sources(
                    source_dir=ZYDIS_ROOT / "src", covered_headers=covered_headers
                )
            )
        )
