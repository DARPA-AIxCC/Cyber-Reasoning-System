...
def make_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--root-disk",
        action="store",
        help="Location of the root disk to use",
    )

    parser.add_argument(
        "--init-path",
        action="store",
        help="Location of the init binary in the VM",
    )
...
def do_it() -> int:
    ...
    # is forced.
    if args.force_9p or args.root_disk:
        use_virtiofs = False
    else:
        ...
        if can_use_microvm(args) and use_virtiofs:
            ...
            arch = virt_arch
    if not use_virtiofs and not args.root_disk:
        export_virtfs(
            qemu, arch, qemuargs, args.root, "/dev/root", readonly=(not args.rw)
        )
    if args.root_disk:
        kernelargs.append("root=/dev/vda")
    ...
    if args.root == "/":
        if args.init_path:
            initcmds = [f"init={args.init_path}"]
        else:
    ...
    if args.root_disk:
        driveid = "root-disk"
        name, fn = sanitize_disk_args("--root-disk", args.root_disk)

        if not Path(fn).exists():
            print("rootdisk does not exist!")
            sys.exit(1)
            #os.makedirs(Path(fn).parent, exist_ok=True)
            #subprocess.run(["qemu-img", "create", "-f", "qcow2", fn, "200G"], check=True)
